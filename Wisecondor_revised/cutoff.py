        ...
        for bin in lookUp[chrom]:
            if len(bin) > 0:
                if float(bin[0][2]) < cutOff:
                    ...
                    removed += 1                        # Is this necessary?????
    return reference,removed

# Return the value of optimal cutoff after filtering once
def getOptimalCutoff(lookUp, repeats, optimalCutoff):
    for i in range(0,repeats):
        reference,removed = getReference(lookUp, optimalCutoff)         # eliminate all ref bins with value >= optimalCutoff
